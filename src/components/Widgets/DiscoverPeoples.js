...
class DiscoverPeoplesWidget extends Component {
    ...
    getDiscoverPeoples() {
        axios.get(`${AppConfig.appUrl}/data/discoverPeoples.js`)
            .then((response) => {
                ...
                {people && people.map((data, key) => (
                    <ListItem button key={key} onClick={() => this.togglePeopleFollow(key)}>
                        <div className="d-flex justify-content-between w-100">
                            ...
                            <div className="d-flex align-items-center">
                                <span className={classnames('badge badge-pill badge-lg', { 'badge-success': data.status, 'badge-default': !data.status })}>{data.status ? 'Following' : 'Follow'}</span>
                            </div>
                        </div>
                    </ListItem>
                ))}
